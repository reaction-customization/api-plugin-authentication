...
import getUserFromFirebaseAuthToken from "./getUserFromFirebaseAuthToken.js";
 ...
 * header is passed to `getUserFromFirebaseAuthToken` and the result is saved on `req.user`.
 *
 * If `getUserFromFirebaseAuthToken` throws an error, a 401 response is sent. If none of the request headers is
...
export default function tokenMiddleware(context) {
  return async (req, res, next) => {
    ...
    try {
      req.user = await getUserFromFirebaseAuthToken(token, context);
      next();
    } catch (error) {
      Logger.error(error);
